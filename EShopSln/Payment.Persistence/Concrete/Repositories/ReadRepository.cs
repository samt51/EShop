...
namespace Payment.Persistence.Concrete.Repositories;
 ...
 public class ReadRepository<T> : IReadRepository<T> where T : class, IBaseEntity, new()
    {
        ...
        public async Task<T?> GetAsync(Expression<Func<T, bool>> predicate, Func<IQueryable<T>, IIncludableQueryable<T, object>>? include = null, bool enableTracking = false,CancellationToken ctx=default)
        {
            IQueryable<T> queryable = Table;
            if (!enableTracking) queryable = queryable.AsNoTracking();
            if (include is not null) queryable = include(queryable);
            ...
            var data = await queryable.FirstOrDefaultAsync(predicate,ctx);
            if (data is null)
            {
